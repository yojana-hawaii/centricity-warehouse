...
drop proc if exists cps_hchp.rpt_EncounterDetails_HFCM;
go
create proc cps_hchp.rpt_EncounterDetails_HFCM
as
begin
	;with u as (
		select 
			c.HF_Case_Manager,c.Last, c.First,c.Name, c.PatientID, c.Last_HF_Enroll_Date, c.Valid_HF_Discharge_Date, d.ObsDate EncounterDate,
		...
		where c.HF_Case_Manager is not null
	)
		select 
			distinct HF_Case_Manager,Last, First,Name, PatientID, Last_HF_Enroll_Date, Valid_HF_Discharge_Date, EncounterDate,
