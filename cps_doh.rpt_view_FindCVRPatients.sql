...
drop view if exists cps_doh.rpt_view_FindCVRPatients
go
create VIEW [cps_doh].[rpt_view_FindCVRPatients]
	AS
	WITH FPExclusive AS (
		SELECT 
	...
	, AllCVRPatients AS (
		...
		from AllCVRPatients cvr
			...
			INNER JOIN [cpssql].CentricityPS.dbo.PatientProfile AS pp ON pp.PId = cvr.PID 
			INNER JOIN [cpssql].CentricityPS.dbo.DoctorFacility AS df ON cvr.usrid = df.PVId 
		WHERE SearchName NOT LIKE 'Test,%'
